...
These tests do NOT call real APIs. Behavior validation (does the model produce the
right answer?) is the eval layer's job and runs against real models with recorded
cassettes from the golden datasets. These tests verify a narrower thing: that the
adapter wires raw SDK responses into the normalized shape correctly. For that we
patch the SDK client with a stub returning a recorded response shape.
"""
...
class _FakeAnthropicMessages:
    async def create(self, **kwargs: Any) -> Any:
        ...
        # default. Contract enforcement for caching being a first-class concern.
